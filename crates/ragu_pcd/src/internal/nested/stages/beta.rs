...
//! committed, so this layout uses a separate stage whose claimed commitment
//! is checked by the parent (see the native `bind_beta` circuit), which holds
//! this proof's `pre_beta` and the claimed point. Connecting that point to the
//! stage polynomial consumed by nested claims requires separate recursive
//! constraints. A nested circuit consuming the endoscalar bits must also
//! constrain their lift to equal this wire.

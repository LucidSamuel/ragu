...
impl<'dr, D: Driver<'dr>> Element<'dr, D> {
    ...
    /// # Errors
    ///
    /// Returns [`Error::InvalidWitness`] if the element is zero, or propagates
    /// any synthesis error from the underlying inverse-check constraints.
    ...
    ///
    /// Returns [`Error::InvalidWitness`] if the element is zero, or propagates
    /// any synthesis error from [`Self::enforce_invertible`].
    ...
    ///
    /// Returns [`Error::InvalidWitness`] if the element is zero, or propagates
    /// any synthesis error from [`Self::enforce_invertible_with`].
    ...
    ///
    /// The prover witnesses `quotient` such that
    /// `quotient * divisor = self`.
    ///
    /// This costs one gate and two constraints.
    ///
    /// # Errors
    ...
    /// constraints used to bind the quotient.

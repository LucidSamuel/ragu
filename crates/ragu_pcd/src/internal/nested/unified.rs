...
//!
//! A parent witnesses all of these in its `preamble` bridge stage and uses
//! the $k(y_n)$ it computes from them as the expected value when folding this
//! step's circuit claims. The [`export`](super::circuits::export) circuit
//! relates this step's instance to its stages. The parent's copies become
//! recursively bound to those values when the parent verifies the nested
//! fold with that expected claim value; constructing the fold prover-side
//! only prepares this check.

...
binding tag, supplied when the registry is finalized. Its purpose is to make
non-trivial evaluations of $s$ unpredictable even to someone who chooses the
circuits, which holds only under the following requirement. The registry tag
($\kappa$) must be sampled independently and without bias after the complete
pre-keyed system description has been fixed and publicly committed, then
permanently bound to that description. The `Tag` documentation in
`ragu_circuits` unpacks each clause.

...
Clean currently supports only prime fields.

Introducing a `variable` means that all subsequent definition will have them as an implicit parameter. This is a convenient way of parametrizing every definition by a concrete circuit prime.
...
> [!TIP]
> Provable types can be also thought as a "structured generic container of objects of type `F`". We will discuss provable types in a later section, the main intuition is that they provide "shape templates" that can be filled with different objects depending on the context.

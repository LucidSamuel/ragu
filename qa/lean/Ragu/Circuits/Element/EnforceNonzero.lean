...
Witnessing the trace implies `input ≠ 0`. The Rust returns
`Nonzero(input)`; the extraction discards the wires, so this circuit
returns `unit`. -/
...
/-- Verifier-side spec: the trace implies `input ≠ 0`. -/

...
During recursive proofs, we often need to commit to data that lives in
the "wrong" field — for example, representing Vesta points (with $\F_q$
coordinates) inside an $\F_p$ circuit.
...
We are in an $\F_p$ circuit with trace polynomial $\v{a}\in\F^n$
and need to hash a commitment $A$ (Vesta point with $\F_q$
coordinates) into the [transcript](./transcript.md). Even with an
algebraic hash function, we cannot hash $\F_q$ elements natively
...
Nested commitments only bind the original data; they do not carry
over any intended operation.
If we want to operate on the original commitment $A\in\G_{host}$
(say $s\cdot A$), we cannot constrain such an operation natively
in an $\F_p$ circuit that only receives
...
multiplication in the $\F_q$ circuit becomes efficient _endoscaling_.
...
These consistency checks constitute two _well-formedness_ requirements:
1. $A(X)$ does not overlap with other partial traces (e.g. $B(X)$)
2. Their commitments add up to the overall trace polynomial commitment

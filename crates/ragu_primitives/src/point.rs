...
impl<'dr, D: Driver<'dr, F = C::Base>, C: CurveAffine> Point<'dr, D, C> {
    ...
    /// # Errors
    ///
    /// Propagates any synthesis error from the intermediate field operations or
    /// from the [`NonzeroBank`] consistency checks if the points do not have
    ...
    ///
    /// Propagates any synthesis error from the intermediate field operations or
    /// from the [`NonzeroBank`] consistency checks if the caller's
